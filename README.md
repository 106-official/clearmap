...
- 修复：随笔/发现流配图与头像无法显示（相对 /api/media 路径自动拼 API_BASE）；选点详情改真全屏独立页（非悬浮卡片）
- 行程：删除景点下拉选择，打卡地点由地图「在此打卡」自动确定；修复「关联的景点不存在」打卡报错
- 产出：dist/ClearMap-cloud-preview-1.04.apk（签名 clearmap2026，内置云后端地址）

## cloud-app-preview-1.03 (2026-09-16)
- 地图：消除卡顿（transform 改走 GPU 复合 + --unit 延迟刷新）；红绿景点分层（整城只显红色重点并自动抽稀、绿色放大浮现）；选点详情改全屏虚化独立页（blur + 居中卡片，不碰底栏）
- 个人：新增简约 SVG 默认头像（不再显示损坏图片框，加载失败自动回退）；修复头像上传后不刷新（相对路径自动拼 API_BASE）；随笔页版面重设计（水墨 Hero + 纸笺书写区）
- 产出：dist/ClearMap-cloud-preview-1.03.apk（签名 clearmap2026，内置云后端地址）

## cloud-app-preview-1.02 (2026-09-16)
- 地图：道路分级 LOD（默认只显深色干道，浅色细分路放大才显）；定位控件上移避开底部栏；拖拽更顺滑（重建阈值放宽）
- 行程：重构为相册/邮戳卡片墙（打卡照片卡与足迹路线卡交叉排列，单屏不整页滚动）；删「记录一段足迹…」文案；修复「开始记录行程」无响应
- 个人：O= 形布局（左滑圆头像 + 右名称/MBTI 徽标/签名，无边框铺底文字）；删 MBTI 大按钮与「写一篇随笔」按钮（迁入「我的随笔」表头）；删按钮蓝色聚焦框
- 修复：补回缺失 #avatarFile 修复头像上传无法保存；uploadAvatar/saveMe/saveMbti/publishEssay 补错误提示
- 产出：dist/ClearMap-cloud-preview-1.02.apk（签名 clearmap2026，内置云后端地址）
